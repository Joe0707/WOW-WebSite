var province =["北京市","天津市","河北省","山西省","内蒙古","辽宁省","吉林省","黑龙江省","上海市","江苏省","浙江省","安徽省","福建省","江西省","山东省","河南省","湖北省","湖南省","广东省","广西自治区","海南省"," 重庆市","四川省","贵州省","云南省","西藏自治区","陕西省","甘肃省","青海省","宁夏回族自治区","新疆维吾尔自治区","香港特别行政区","澳门特别行政区","台湾省","其它"];
	var city=[["东城区","西城区","崇文区","宣武区","朝阳区","丰台区","石景山区"," 海淀区（中关村）","门头沟区","房山区","通州区","顺义区","昌平区","大兴区","怀柔区","平谷区","密云县","延庆县"," 其他"],
...
$(function(){
	$(".text").html(
			'<form id="text" action="/WOW/modify.do">'
			+'<input id="provalue"type="hidden" name="loc" value="'+loc+'" />'
			+'<p>账号:'+account+''
			+'</p>'
			...
			+'</select>'
			+'</p>'
			+'<p>省份:<select class="loca" onchange="prochange();">'
			/*+'<option '+choose("北京市",loc)+'>北京市</option>'
			+'<option '+choose("天津市",loc)+'>天津市</option>'*/
			+'</select>'
			+'城市<select class="city" name="city">'
			//+'<option '+choose("北京市",loc)+'>北京市</option>'
			//+'<option '+choose("天津市",loc)+'>天津市</option>'
			+'</select>'
			+'</p>'
			...
			+'<input class="res" type="reset" value="重置修改" onclick="getback();"/>'
			+'</form>'
	)
	var cit = $('<option selected>'+cityvalue+'</option>');
	$(".city").append(cit);
	//var b =$(".loca").html();
	//console.log(b);
	for(var i = 0;i<province.length;i++){
		if(province[i]==loc){
		var opt = $('<option selected value="'+i+'">'+province[i]+'</option>');
		$(".loca").append(opt);
		}else{
			var opt = $('<option value="'+i+'">'+province[i]+'</option>');
			$(".loca").append(opt);
		}
	}
	console.log($("#provalue")[0].value);
})
function getback(){
	$(".city").html('');
	var cit = $('<option selected>'+cityvalue+'</option>');
...
function prochange(){
	...
	$("#provalue")[0].value=province[$(".loca")[0].value];
	console.log($("#provalue")[0].value);
}
function showperson(){
	$(".text").html(
